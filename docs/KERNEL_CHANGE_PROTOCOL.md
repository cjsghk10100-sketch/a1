...
- [ ] 이벤트/계약 변경 시 `docs/EVENT_SPECS.md`를 함께 갱신했는가
- [ ] `schemaVersion.ts`는 커널 가드 요구사항을 충족하도록 touch했는가
  - MAJOR/MINOR에서 버전 정책 변경이 필요할 때만 bump + `SUPPORTED_VERSIONS` 갱신
  - 단순 additive 커널 변경은 버전 고정 가능(호환성 유지 시)
