...
7. **반복 검증 우선**: 단발 의견 채택으로 끝내지 않고, **일론 머스크식 1원칙 분해 + 취약점 탐지**를 결합한 교차검증을 수행한다. 중요한 결정은 상반 관점(문서/에이전트/가정) 왕복 검증을 **최소 4회 이상** 반복한 뒤에만 최종안을 확정한다.
8. **문제 흡수 루프 고정**: 큰 사고를 0으로 단정하지 않고, `깨짐 탐지 → 소단위 수정 → 재검증 → 헌법/정책 반영` 루프를 상시 운영한다. 목표는 무오류 선언이 아니라 **문제 흡수 속도가 문제 발생 속도보다 빠른 상태**를 유지하는 것이다.

### 1.1 SSOT 계층 정의

### 1.1.1 [부록] 하이브리드 OS SSOT 및 상태 전이 규칙 (v1.1)
에이전트 앱(이벤트 스토어)과 로컬 파일 시스템을 통합 운영하기 위한 절대 원칙이다.

1. 단일 진실 원본 (The Only SSOT)
- 시스템의 모든 상태와 진실은 **오직 `앱 이벤트 스토어(evt_events)`**에만 존재한다.
- `memory/state.md` 및 `04_WORKFLOW_PIPELINE` 내부의 파일 위치는 진실이 아니며, 앱 상태를 읽기 편하게 보여주는 **그림자(Projection/View)**에 불과하다.
...
## 16. 교차 검증 합의 루프 우선 원칙
- 중요한 아키텍처/보안/운영 결정은 1회 제안으로 확정하지 않는다.
- 검토 방식은 **일론 머스크식 1원칙 분해(문제·제약·핵심 메커니즘·검증 지표)**를 기본으로 하고, 매 회차마다 **취약점 탐지(보안/무결성/운영 리스크)**를 병행한다.
- 최소 2개 이상의 관점(상반 문서/에이전트 의견/가정 검증)을 왕복 검토해 반례와 취약점을 표면화하며, 이 과정을 **최소 4회 이상 반복**한다.
- 최종안은 반드시 `제약 우선(보안·무결성·재현성) → 실행 순서` 형태로 고정한다.
- 사용자가 반복 검토를 요청한 경우, 같은 문서를 재독하고 변경된 판단 근거를 명시해 드리프트를 차단한다.

## 17. A2A 전달 신뢰성 규칙 (ack/retry/fallback)
- 에이전트 간 중요 요청은 수신 즉시 `ACK`(수신 확인)를 남긴다.
- 지정 시간 내 ACK 또는 결과가 없으면 자동 재전송을 최대 2회 수행한다.
- 재시도 후에도 무응답이면 세션을 직접 지정해 수동 재요청한다.
- 최종 실패 시 `incident`로 기록하고, 사용자에게 현재 상태(대기/재시도/실패)를 보고한다.
- 무응답은 고의 무시로 간주하지 않으며, 전달 계층(라우팅/타임아웃) 이슈로 처리한다.

## 18. 점진 이관 고정 원칙
- 새 작업은 신구조(AGENT_OS_WORKSPACE 기준 경로/프로토콜)로 시작한다.
- 기존 자산은 즉시 전면 이동하지 않고, 주간 배치로 검증 통과분만 순차 이관한다.
- 기존 파일은 이관 전까지 링크/참조 방식으로 유지해 운영 중단을 방지한다.
- 무중단 점진 이관 원칙을 위반하는 전면 교체 제안은 기본 반려한다.
