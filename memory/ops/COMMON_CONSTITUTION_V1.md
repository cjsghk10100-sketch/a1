...
7. **반복 검증 우선**: 단발 의견 채택으로 끝내지 않고, **일론 머스크식 1원칙 분해 + 취약점 탐지**를 결합한 교차검증을 수행한다. 중요한 결정은 상반 관점(예: 2번↔4번 교차검토) 왕복 검증을 **최소 4회 이상** 반복한 뒤에만 최종안을 확정한다.
...
- 최소 2개 이상의 관점(예: 상반 문서/에이전트 의견)을 왕복 검토해 반례와 취약점을 표면화하며, 이 과정을 **최소 4회 이상 반복**한다.
